<template>
  <div class="w-full max-w-full overflow-x-auto">
    ...
    <div class="flex flex-col gap-4 p-2 rounded-lg transition-all duration-300">
      <div v-for="location in enabledLocations" :key="location.id" :class="[
        ...
        <div
          ...
          <span class="font-semibold text-sm text-green-900 mb-1 sm:mb-0"> {{ location.desa }} </span>
        ...
        <div class="flex gap-2 w-full overflow-y-auto">
          <WeatherBox v-for="timeSlot in timeSlots" :key="`${location.id}-${timeSlot.hour}`"
            :weather-data="getWeatherBoxData(location.id, timeSlot)" :loading="isLoading(location.id)"
            :error="getError(location.id)" :time-slot="timeSlot" />
        </div>
      </div>
    </div>

    <!-- Empty State -->
    <div v-if="enabledLocations.length === 0"
      ...
      <p class="text-green-800 text-lg m-0">No locations enabled. Please enable locations to view weather data.</p>
    </div>
  </div>
</template>
